<?php
	...
	if(isset($_POST['confirm_update_subject'])) {
	
		$subject = new subjects();
		
		$args = array(
			"subject_name" => FILTER_SANITIZE_STRING,
			"maktab_course" => FILTER_VALIDATE_INT,
			"subject_desc" => FILTER_SANITIZE_STRING,
			"subject_id" => FILTER_VALIDATE_INT
		);
		
		$postData = filter_var_array($_POST,$args);	
		
		$subject->updateSubject($postData);
        ...
        <div class="row">
          <div class="col-lg-12">
			...
			<?php 
				...
				$getData = filter_var_array($_GET,array("subject_id"=>FILTER_VALIDATE_INT));
				
				$subjectDetails = $subject->getSubjectById($getData['subject_id']);
			?>
        <div class="row">
          <div class="col-lg-6">
            ...
            <form role="form" method="post" action="" onsubmit="return validateSubjects();">
			  <div class="form-group">
                ...
                <input type="text" class="form-control" name="subject_name" placeholder="Enter text" value="<?php echo $subjectDetails['subject_name']; ?>">
			   ...
			   <div class="form-group">
                ...
                <select class="form-control" name="maktab_course" placeholder="Enter text">
				 ...
				 foreach($coursedetails as $value){
					...
					if($value['course_id'] == $subjectDetails['mst_course_course_id']) {
			...
			<div class="form-group">
                ...
                <textarea class="form-control" placeholder="Enter text" rows="3" name="subject_desc" ><?php echo $subjectDetails['subject_description']; ?></textarea>
                <p class="help-block"></p>
             </div>
			  
			<input type="hidden" name="subject_id" value="<?php echo $_GET['subject_id']; ?>">
			<input type="submit" class="btn btn-default" name="confirm_update_subject" value="Update Subject">
